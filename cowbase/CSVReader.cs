  ...
  public class CSVReader : IDisposable {
    ...
    private void ParseCSVFields(ArrayList result, string data) {
      ...
      while (pos < data.Length)
        result.Add(UnescapeCharacters(ParseCSVField(data, ref pos)));
    }


    private string UnescapeCharacters(string inString)
    {
        string outString = string.Empty;
        char[] inCharacters = inString.ToCharArray();
        char[] hex = new char[2];
        int inLength = inCharacters.Length;
        char inCharacter;
        

        for (int i = 0; i < inLength; i++)
        {
            inCharacter = inCharacters[i];
            if (inCharacter == '%')
            {
                if(i+2 < inLength)
                {
                    hex[0] = inCharacters[i + 1];
                    hex[1] = inCharacters[i + 2];
                    outString += Convert.ToChar(HexToInt(hex));
                    i += 2;
                }
                else if (i + 1 < inLength)
                {
                    hex[0] = '0';
                    hex[1] = inCharacters[i + 1];
                    outString += Convert.ToChar(HexToInt(hex));
                    i += 1;
                }               
    ...
    private int HexToInt(char[] hex)
    {
        ...
        {
            result = 16 * result + HexValues.IndexOf(hex[i]);
        }
        return result;
    }
